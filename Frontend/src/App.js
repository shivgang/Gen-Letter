...
function App() {
  ...
  const getUser = async () => {
    try {
      const url = `https://genletter-pa28.onrender.com/auth/login/success`;
      const { data } = await axios.get(url, { withCredentials: true });
      console.log(data);
   
      await axios.get('https://genletter-pa28.onrender.com/resume/history/')
        .then((response) => {
      ...
      await axios.get('https://genletter-pa28.onrender.com/offer_letter/history/')
        .then((response) => {
  ...
  return (
    <BrowserRouter>
      {window.location.href !== "https://genletter-pwt.netlify.app/login" && (
        <Navbar user={user} />
      )}
      <Routes>
        <Route
